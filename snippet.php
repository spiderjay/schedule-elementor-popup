<?php
	...
	$dt = new DateTime( $kffa_popup_control['show'] );	
	$dt->setTimezone(new DateTimeZone($tz));
	...
	$dt = new DateTime( $kffa_popup_control['hide'] );	
	$dt->setTimezone(new DateTimeZone($tz));
	...
	if ( $ts_now > $ts_show && $ts_now < $ts_hide ){
		$post = array( 'ID' => $kffa_popup_control['id'], 'post_status' => 'publish' );
		wp_update_post($post);		
	}
	
	// hide post/popup if time doesn't match
	if ( $ts_now > $ts_hide || $ts_now < $ts_show ){
		$post = array( 'ID' => $kffa_popup_control['id'], 'post_status' => 'draft' );
		wp_update_post($post);
	}
